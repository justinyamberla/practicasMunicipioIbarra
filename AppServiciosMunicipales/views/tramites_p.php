...
if (isset($_GET['idDireccion'])) {
    ...
    $sql_por_dir = "SELECT * FROM public.tramites where id_direccion = '$id_direccion'
               AND tipo = 'Presencial' ORDER BY id_tramite ASC ";
}
if (isset($_GET['txtPalabra'])) {
    $palabra_clave = strtoupper($_GET['txtPalabra']);
    $sql_por_palabra = "SELECT * FROM public.tramites where (nombre ilike '%$palabra_clave%' 
                        or translate(nombre, 'áéíóúÁÉÍÓÚäëïöüÄËÏÖÜñ', 'aeiouAEIOUaeiouAEIOUÑ') 
                        ilike '%$palabra_clave%') AND tipo = 'Presencial' order By nombre";
}
